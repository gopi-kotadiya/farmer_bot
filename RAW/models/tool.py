...
class ToolParam(BaseModel):
    ...
    def to_property(self):
        ...
        if self.enums: prop["enum"] = self.enums
        if self.items: prop["items"] = self.items
        return prop
...
class Tool(BaseModel):
    ...
    function: Any  # Union types can be added later as needed
    ...
    def to_dict(self):
        ...
        for param in self.parameters:
            ...
            if param.required: required_fields.append(param.name)
        return {
            "type": "function",
            "function": {
                "name": self.name,
